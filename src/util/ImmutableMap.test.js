...
test("get absolute cell reference", () => {
    const a1 = SpreadsheetCellReference.parse("$A$1");
...
test("get unknown key", () => {
    ...
    expect(
        map.get(SpreadsheetCellReference.parse("Z99"))
    ).toBeUndefined();
});
...
test("remove", () => {
    ...
    expect(map.remove(SpreadsheetCellReference.parse("A1")))
        .toStrictEqual(
            new ImmutableMap(
                new Map(
                    [
...
test("fromJson not empty", () => {
    ...
    expect(
        map.get(SpreadsheetCellReference.fromJson("A1"))
    ).toStrictEqual(SpreadsheetColumnReference.parse("B"));
    expect(
        map.get(SpreadsheetCellReference.fromJson("B2"))
    ).toStrictEqual(SpreadsheetColumnReference.parse("C"));
    expect(
        map.toJson()
    ).toStrictEqual({
        "A1": "B",
